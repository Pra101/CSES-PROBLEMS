...
int32_t main(){
	...
	//vi dp(limit+1,-1);
	for(int i=1;i<=limit+1;i++)dp[i][0]=-1;
	dp[0][0]=0;
	...
	for(int i=0;i<n;i++){
		...
		//trace(dp);
		for(int j=0;j+price[i]<=limit+1;j++){
			if(dp[j][0]!=-1)
			dp[j+price[i]][1]=max(dp[j+price[i]][1],(dp[j][0]+pages[i]));
		}
		//dp=dp1;
		for(int k=0;k<=limit;k++){
			dp[k][0]=dp[k][1];
	...
	int mx_=0;
	for(int i=0;i<=limit;i++)mx_=max(mx_,dp[i][0]);
	cout<<mx_;
