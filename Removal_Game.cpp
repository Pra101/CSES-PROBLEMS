//CSES 
//Link to this code: https://cses.fi/paste/7bf582202b2f98471fe010/
// Author: pra101 
#include <bits/stdc++.h>
	...
	for(int gap=1;gap<n;gap++){
		for(int i=0,j=gap;i+gap<n;i++,j++){
			...
			if(arr[i]+dp[i+1][j][1]>=arr[j]+dp[i][j-1][1]){
				dp[i][j][0]=arr[i]+dp[i+1][j][1];// for 1st best score
				dp[i][j][1]=dp[i+1][j][0]; // for 2nd best score
			}
			else {
				dp[i][j][0]=arr[j]+dp[i][j-1][1]; // for 1st best score
				dp[i][j][1]=dp[i][j-1][0]; // for 2nd best score
			}
		}
	}
	//~ for(int i=0;i<=n;i++){
